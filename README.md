...
- **Music of the desk.** An original, *evolving* electronic score — and **no two trials are the same piece**. Each trial is freshly seeded: a different key, a different tempo, one of several chord progressions, a different timbre. Within it the music moves through four cycling phases — **drift** (lush dreamy pads), **pulse** (deep hypnotic Berlin-ish house — soft round kick, offbeat dub-chord stabs, sidechained "breathing" pads), **lift** (a progressive driving arpeggio), **ebb** (a breakdown). It *reacts*: pace and brightness climb with your **fervor**; tension gathers — top-end, a rising lift — as the **glass empties**. It plays on continuously, and when a trial concludes it turns to a calm, resolved **coda** for the post-summary — its own piece, never just the same loop. Fully synthesised (Web Audio, no files, no samples), with its **own volume** apart from the effects, and toggleable at the desk.
